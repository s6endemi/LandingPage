...
export function WaitlistModal({ onClose }: WaitlistModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      trackSignupError("waitlist-modal", "Unexpected error: " + error.toString());
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onKeyDown={handleKeyDown}>
      ...
      >
        ...
        {isMobile && <div className="w-12 h-1 bg-gray-300 rounded-full mx-auto mt-2 mb-1"></div>}
            ...
            {!submitted ? (
              ...
              >
                ...
                <p className="text-sm sm:text-base text-gray-600 text-center mb-5 sm:mb-6">
                  Sei einer der Ersten mit Zugang zum Athly KI-Coach. Limitierte Plätze verfügbar.
                </p>
                ...
                <form onSubmit={handleSubmit} className="mb-5 sm:mb-6">
                  <div className="mb-3">
                    <div
                      className={`relative rounded-xl overflow-hidden border transition-all ${focused ? "border-[#9bc539] ring-2 ring-[#9bc539]/20" : "border-gray-200"}`}
                    >
                      <input
                        ref={inputRef}
                        type="email"
                ...
                <div className="space-y-2 sm:space-y-3 mb-5 sm:mb-6">
                  <p className="text-center text-xs sm:text-sm text-gray-500 mb-2 sm:mb-3">
                    Mit deiner Anmeldung erhältst du:
                  </p>

                  <div className="flex items-start sm:items-center">
                    <svg
                      className="w-4 h-4 mr-2 sm:mr-3 text-[#9bc539] flex-shrink-0 mt-0.5 sm:mt-0"
                      viewBox="0 0 24 24"
                      fill="currentColor"
                    >
                    ...
                    <span className="text-xs sm:text-sm text-gray-700">
                      <strong>Pionier-Bonus</strong> weil dein frühes Vertrauen unbezahlbar ist
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-center">
                    <div className="flex -space-x-1.5 sm:-space-x-2 mr-2">
                      {[...Array(4)].map((_, index) => (
                        <div
                          key={index}
                          className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-white overflow-hidden bg-gray-200"
                        ></div>
                      ))}
              ...
              >
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full mx-auto flex items-center justify-center mb-4 sm:mb-6 bg-[#9bc539]/10">
                  <svg
                    className="w-7 h-7 sm:w-8 sm:h-8 text-[#9bc539]"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                ...
                <p className="text-sm sm:text-base text-gray-600 text-center mb-5 sm:mb-6">
                  Wir haben deine E-Mail-Adresse erhalten und informieren dich, sobald dein exklusiver Zugang bereit
                  ist.
                </p>
        ...
        <div className="py-3 sm:py-4 px-5 sm:px-6 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-2 text-center sm:text-left">
          <div className="flex items-center">
            ...
            {[1, 2, 3, 4, 5].map((star) => (
              <svg
                key={star}
                className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-[#FBBF24]"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
            ...
            <span className="ml-1.5 text-xs font-medium text-gray-800">4.8/5</span>
          </div>

          {/* Security assurance */}
          <div className="flex items-center">
            <svg
              className="w-3.5 sm:w-4 h-3.5 sm:h-4 mr-1.5 text-gray-800"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="1.5"
